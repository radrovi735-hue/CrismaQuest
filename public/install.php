...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$installed) {
    ...
    foreach (['db_host','db_name','db_user','admin_email'] as $field) {
        if ($values[$field] === '') { $errors[] = 'Preencha todos os campos obrigatórios.'; break; }
    }
    if (!filter_var($values['admin_email'], FILTER_VALIDATE_EMAIL)) $errors[] = 'Informe um e-mail válido para o administrador.';
    if (strlen($adminPassword) < 8) $errors[] = 'A senha do administrador precisa ter pelo menos 8 caracteres.';
    if ($adminPassword !== $adminPasswordConfirm) $errors[] = 'As duas senhas do administrador não coincidem.';
    if (!is_readable($sqlPath)) $errors[] = 'A base de instalação não foi encontrada.';
    ...
    if ($errors === []) {
        try {
            ...
            if ($sql === false) throw new RuntimeException('Não foi possível ler a base SQL.');
            ...
            $stmt->execute(['password'=>password_hash($adminPassword,PASSWORD_DEFAULT),'email'=>$values['admin_email']]);
            if ($stmt->rowCount() < 1) {
                $exists = $pdo->query("SELECT COUNT(*) FROM ct_utenti WHERE username='admin'")->fetchColumn();
                if ((int)$exists < 1) throw new RuntimeException('O usuário administrador não existe na base importada.');
            }
            ...
            if (file_put_contents($envPath, $env) === false) throw new RuntimeException('Não foi possível criar o arquivo de configuração.');
            if (!is_dir($root.'/var')) @mkdir($root.'/var',0775,true);
...
function envVal(string $v): string { return '"'.str_replace(['\\','"'],['\\\\','\\"'],$v).'"'; }
function splitSql(string $sql): array {
    $sql = preg_replace('/^\xEF\xBB\xBF/','',$sql) ?? $sql;
    $out=[]; $buf=''; $quote=null; $line=false; $block=false; $len=strlen($sql);
    for($i=0;$i<$len;$i++){
        $c=$sql[$i]; $n=$i+1<$len?$sql[$i+1]:'';
        if($line){ if($c==="\n"){$line=false;$buf.="\n";} continue; }
        if($block){ if($c==='*'&&$n==='/'){$block=false;$i++;} continue; }
        if($quote!==null){ $buf.=$c; if($c==='\\'&&$n!==''){$buf.=$n;$i++;continue;} if($c===$quote&&$n===$quote){$buf.=$n;$i++;continue;} if($c===$quote)$quote=null; continue; }
        if($c==='#'){ $line=true; continue; }
        if($c==='-'&&$n==='-'&&($i+2>=$len||ctype_space($sql[$i+2]))){$line=true;$i++;continue;}
        if($c==='/'&&$n==='*'){$block=true;$i++;continue;}
        if($c==="'"||$c==='"'||$c==='`'){$quote=$c;$buf.=$c;continue;}
        if($c===';'){$out[]=$buf;$buf='';continue;}
        $buf.=$c;
    }
    if(trim($buf)!=='')$out[]=$buf; return $out;
}
?>
<!doctype html>
<html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Instalar CrismaQuest</title>
<style>
...
</style></head><body><div class="wrap"><div class="hero"><div class="brand">Crisma<span>Quest</span></div><p>Uma jornada de participação, descoberta e comunidade.</p></div><div class="card">
<?php if($installed && !$success): ?><div class="alert ok"><strong>CrismaQuest já está instalado.</strong><br><a href="/loginStud">Abrir login dos crismandos</a> · <a href="/loginDoc">Abrir painel do catequista</a></div>
<?php elseif($success): ?><div class="alert ok"><strong>Instalação concluída.</strong> O banco e o administrador foram configurados.</div><a class="btn" style="display:block;text-align:center;text-decoration:none" href="/loginDoc">Entrar no CrismaQuest</a>
<?php else: ?>
<h1 style="font:700 27px Georgia,serif;margin-top:0">Configuração inicial</h1><p class="hint">Use os dados mostrados em “MySQL Databases” no InfinityFree. Não é necessário configurar e-mail/SMTP para começar.</p>
...
<div class="field"><label>Senha MySQL</label><input name="db_password" type="password" value=""></div>
...
</div><button class="btn" type="submit">Instalar CrismaQuest</button></form><?php endif; ?></div><div class="parish">Paróquia Nossa Senhora dos Remédios · Arquidiocese de Fortaleza</div></div></body></html>
